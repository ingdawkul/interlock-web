...
export default function TrendViewer({ trendData, initialParam }) {
  ...
  // ── Sync selected when opened from Report ──
  useEffect(() => {
    if (initialParam) setSelected(initialParam);
  }, [initialParam]);

  // ── Reset visible machines when selected param changes ──
  useEffect(() => {
    if (!selected) return;
  ...
  // ── Scroll selected param into view in the list ──
  useEffect(() => {
    if (!selected) return;
    const el = document.getElementById(`param-${CSS.escape(selected)}`);
  ...
  const chartData = useMemo(() => {
    if (xAxisMode === "index") return uniformIndexedData;
  ...
  function TimeTooltip({ active, payload }) {
    ...
    return (
      <div style={{ background: "#111", padding: "10px", borderRadius: "6px", color: "#fff" }}>
        {item.name && (
          <div style={{ fontSize: 18, fontWeight: 700, color: item.stroke, marginBottom: 4 }}>
            {item.payload.machine || item.name}
          </div>
        )}
        {date && (
          <div style={{ fontSize: 14, fontWeight: 500, opacity: 0.85, marginBottom: 4 }}>
            {date.toLocaleDateString("no-NO")}{" "}
            {date.toLocaleTimeString("no-NO", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
          </div>
        )}
        <div style={{ fontSize: 16 }}>
          Avg: {data.avg}
          {showRange && data.min != null && data.max != null && (
            <><div>Min: {data.min}</div><div>Max: {data.max}</div></>
          )}
        </div>
      </div>
    );
        ...
        {payload.map((entry, i) => {
          ...
          return (
            <div key={i} style={{ marginBottom: 6 }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: entry.stroke }}>{machine}</div>
              {date && (
            ...
            <>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {Object.keys(visibleDataByMachine).length === 0
                  ? <div className="h-full flex items-center justify-center text-gray-400">No machines selected</div>
                  : <LineChart key={chartKey} data={chartData}>
                        ...
                        <>
                          ...
                          {["min", "max"].map(k =>
                            thresholdConfig[k] !== undefined && (
                              <ReferenceLine key={k} y={thresholdConfig[k]} stroke="#DC2626" strokeWidth={3}
                                label={{ value: `${k.charAt(0).toUpperCase() + k.slice(1)} ${thresholdConfig[k]}${unit ? " " + unit : ""}`, position: "right", fill: "#DC2626", fontSize: 12 }}
                              />
                            )
                          )}
                          {["warningMin", "warningMax"].map(k =>
                            thresholdConfig[k] !== undefined && (
                              <ReferenceLine key={k} y={thresholdConfig[k]} stroke="#F59E0B" strokeWidth={3}
                                label={{ value: `Warn ${thresholdConfig[k]}${unit ? " " + unit : ""}`, position: "right", fill: "#F59E0B", fontSize: 11 }}
                              />
                            )
                          )}
                        </>
                      ...
                      {xAxisMode === "time" ? (
                        <XAxis dataKey="timestamp" type="number" scale="time" domain={["auto", "auto"]} tick={<TimeTick />} allowDataOverflow={false} />
                      ) : (
                        <XAxis dataKey="index" type="number" scale="linear" domain={xDomain} allowDataOverflow={false} tickFormatter={v => `#${v}`} tick={{ fontSize: 11 }} />
                      )}
                      ...
                      <Legend
                        onClick={e => {
                          ...
                          setVisibleMachines(prev => {
                            ...
                            const isOnlyOneVisible = machines.filter(m => prev[m]).length === 1 && prev[key];
                            if (isOnlyOneVisible) {
                              const reset = {};
                        ...
                        ? Object.keys(visibleDataByMachine).map((machine) => {
                            ...
                            return (
                              <React.Fragment key={machine}>
                                {showRange && (
                                  <>
                                    <Line data={uniformIndexedData} type="monotone" dataKey={d => d[machine]?.min ?? null} stroke={color} strokeDasharray="4 4" dot={false} legendType="none" isAnimationActive={true} />
                                    <Line data={uniformIndexedData} type="monotone" dataKey={d => d[machine]?.max ?? null} stroke={color} strokeDasharray="4 4" dot={false} legendType="none" isAnimationActive={true} />
                                  </>
                                )}
                                <Line data={uniformIndexedData} type="monotone" dataKey={d => d[machine]?.avg ?? null} name={machine} stroke={color} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} connectNulls={false} isAnimationActive={true} />
                              </React.Fragment>
                            );
                          })
                        : Object.entries(visibleDataByMachine).map(([machine, points]) => {
                            ...
                            return (
                              <React.Fragment key={machine}>
                                ...
                                <Line data={pointsWithIndex} type="monotone" dataKey="avg" name={machine} stroke={color} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} connectNulls={false} isAnimationActive={true} />
                              </React.Fragment>
                            );
                          })
                      }
