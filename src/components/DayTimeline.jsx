...
function buildPowerIntervals(events) {
  ...
  for (const e of sorted) {
    if (e.type === "OFF") active = { start: e.time }
    if (e.type === "ON" && active) {
      intervals.push({ start: active.start, end: e.time })
      active = null
    }
  }
  return intervals
  ...
  return (
    <div className="w-full relative select-none">
      ...
      <div className="relative h-12 rounded-full overflow-hidden bg-gray-300 z-0">
        ...
        {/* Off-hours */}
        <div className="absolute top-0 bottom-0 bg-gray-400"
          style={{ left: 0, width: toPct(workStartMin) }} />
        <div className="absolute top-0 bottom-0 bg-gray-400"
          style={{ left: toPct(workEndMin), width: toPct(DAY_MINUTES - workEndMin) }} />

        {/* Work hours */}
        <div className="absolute top-0 bottom-0 bg-green-500"
          style={{ left: toPct(workStartMin), width: toPct(workEndMin - workStartMin) }} />
